...
u8 RTC_Init(void){
	...
	if(BKP->DR1!=0x5050){
		...
		RCC->APB1ENR|=1<<28;     //使能电源时钟	   RCC_APB1Periph_PWR 
		RCC->APB1ENR|=1<<27;     //使能备份时钟	   RCC_APB1Periph_BKP  
		
		//设置寄存器PWR_CR的DBP位，使能对后备寄存器和RTC的访问
		PWR->CR|=1<<8;           //取消备份区写保护
		
		//RTC模块和时钟配置系统(RCC_BDCR寄存器)处于后备区域
		RCC->BDCR|=1<<16;        //备份区域软复位	   
		RCC->BDCR&=~(1<<16);     //备份区域软复位结束	  	 
	  RCC->BDCR|=1<<0;         //开启外部低速振荡器 
		while((!RCC->BDCR&0x20)&&temp<250){//检查指定的RCC标志位设置与否,等待外部时钟就绪	
			temp++;
			delay_ms(10);
		}
    ...
    while(!(RTC->CRL&(1<<5)));//等待RTC寄存器操作完成	 
		...
		RTC->PRLH=0X0000;         //设置RTC预分频的值(PRLH and PRLL)
...
//获取unix时间戳，由CNTHh和CNTL两个寄存器的值组成
